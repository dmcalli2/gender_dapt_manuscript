...
```{r}
## Add all labels back into to br table to allow index referencing for abstract
br_reference <- br
empty <- which(br_reference$Sex == "")
br_reference$Sex[empty[1:(0.5*length(empty))]] <- "Female"
empty <- which(br_reference$Sex == "")
br_reference$Sex[empty] <- "Male"
...
Methods
------
We searched Ovid MEDLINE, EMBASE and the Cochrane Central Register of Controlled Trials (CENTRAL) for randomised controlled trials of oral P2Y~12~ inhibitors for stroke or ACS. Age and sex-specific mortality outcomes were obtained for all patients admitted to hospital with myocardial infarction in Scotland from 2006 to 2010 (prior to introduction of prasugrel or ticagrelor).

Results
-------

From 7,148 articles, 9 fulfilled our inclusion criteria. Two stroke and 3 ACS trials compared clopidogrel to placebo, 1 stroke trial compared ticagrelor to aspirin, and 3 ACS trials compared newer P2Y12 inhibitors to clopidogrel.  In the latter comparison, the treatment rate ratio (RR) for major adverse cardiovascular events in men was `r men_q[1]` (95% CI `r men_q[2]` to `r men_q[3]`). For the same outcome, across all 9 trials the sex-treatment interaction RR was `r inter_q["Pooled", "interaction"]`. Combining these estimates yielded a treatment RR in women of `r women_q["Pooled", "50%"]` (95% CI `r women_q["Pooled", "2.5%"]` to `r women_q["Pooled", "97.5%"]`).
...
Despite finding a`r inter_p["re"]` probability that the relative treatment efficacy was lower in women than men, the probability that the absolute risk reduction for all-cause mortality was also lower was only `r arr_prob` .
...
forest_plot
tiff("figures/figure2.tiff", width = 14, height = 7, units = "in", res = 600, compression = "lzw")
forest_plot
dev.off()
```
